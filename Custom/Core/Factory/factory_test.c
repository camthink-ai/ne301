...
/* One-shot identity regeneration at MAC-burn / MAC-clear time. Both are
 * factory-only paths that run before the device reaches a user, so the
 * MAC-derived defaults (device name / AP SSID / MQTT client-id + topics)
 * are reset to their sentinels right here; the regular boot-time
 * default-generation paths then refill them from whatever MAC the system
 * ends up using (burned factory MAC, or the chip-reported one after a
 * clear). No staleness pattern checks run at boot — user customizations
 * are impossible before the burn by construction. */
static void factory_identity_reset_sentinels(void)
{
    /* Device name: back to the sentinel — the boot-time updater regenerates
     * it (and stamps the live MAC into device_info) from the MAC actually
     * in use after the reboot that applies the change. The persisted MAC is
     * reset to the not-stamped default too: on clear it still holds the
     * burned string, and the load-time name regeneration would otherwise
     * rebuild the name from the OLD burn. */
    device_info_config_t dev_info;
    if (json_config_get_device_info_config(&dev_info) == AICAM_OK) {
        snprintf(dev_info.device_name, sizeof(dev_info.device_name), "AICAM-000000");
        snprintf(dev_info.mac_address, sizeof(dev_info.mac_address), "00:00:00:00:00:00");
        (void)json_config_set_device_info_config(&dev_info);
    }
    ...
    /* MQTT identity: reset to the sentinels — mqtt_build_topics regenerates
     * them from the device MAC on the next service start. */
    mqtt_service_config_t mqtt_cfg;
    if (json_config_get_mqtt_service_config(&mqtt_cfg) == AICAM_OK) {
        snprintf(mqtt_cfg.base_config.client_id, sizeof(mqtt_cfg.base_config.client_id), "AICAM-000000");
        mqtt_cfg.data_receive_topic[0] = '\0';
        mqtt_cfg.data_report_topic[0] = '\0';
        mqtt_cfg.telemetry_topic[0] = '\0';
        (void)json_config_set_mqtt_service_config(&mqtt_cfg);
    }

    LOG_SIMPLE("Identity reset (name/SSID/MQTT regenerate on next boot)\r\n");
}

static void factory_identity_reset(const uint8_t mac[6])
{
    char mac_str[18];
    snprintf(mac_str, sizeof(mac_str), "%02X:%02X:%02X:%02X:%02X:%02X",
             mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

    factory_identity_reset_sentinels();
    /* Push the new MAC through the updater — it persists the MAC into
     * device_info so the identity switch is visible before the reboot. */
    (void)json_config_update_device_mac_address(mac_str);
}
...
/* Shared validity rule for a burned MAC: neither the erased (0xFF) nor the
 * empty (0x00) flash pattern, and the multicast bit must be clear. */
static int factory_mac_is_valid(const uint8_t *mac) {
    uint8_t acc_or = 0x00, acc_and = 0xFF;
    for (int i = 0; i < 6; i++) {
        acc_or |= mac[i];
        acc_and &= mac[i];
    }
    return (acc_or != 0x00 && acc_and != 0xFF && !(mac[0] & 0x01)) ? 0 : -1;
}
...
int factory_mac_get_burned(uint8_t *mac) {
    ...
    if (storage_nvs_read(NVS_FACTORY, NVS_KEY_MAC_ADDR, mac, 6) != 6) {
        return -1;
    }
    return factory_mac_is_valid(mac);
}
...
static int factory_test_cmd(int argc, char *argv[]) {
    if (argc < 2) {
        ...
        LOG_SIMPLE("  mac [XX:..|clear]              - Show/burn/clear factory MAC\r\n");
    ...
    if (strcmp(cmd, "mac") == 0) {
        uint8_t mac[6];
        if (argc < 3) {
            if (factory_mac_get_burned(mac) == 0) {
                LOG_SIMPLE("Factory MAC: %02X:%02X:%02X:%02X:%02X:%02X (burned)\r\n",
                           mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
                LOG_SIMPLE("(STA uses it, AP derives +1; identity follows it)\r\n");
            } else {
                LOG_SIMPLE("Factory MAC: not burned\r\n");
                LOG_SIMPLE("(chip-reported MAC in use)\r\n");
            }
            return 0;
        }
        ...
        if (strcmp(argv[2], "clear") == 0) {
            ...
            /* The name/SSID/MQTT values regenerated from the burned MAC are
             * no longer sentinels — without this reset they would outlive
             * the burn while the interfaces revert to the chip MAC. The
             * boot-time updater then re-stamps device_info MAC + name from
             * the live (chip) MAC. */
            factory_identity_reset_sentinels();
            LOG_SIMPLE("Factory MAC cleared\r\n");
            LOG_SIMPLE("(chip-reported MAC + identity regenerate on next boot)\r\n");
            return 0;
        }
        // Burn: factory mac XX:XX:XX:XX:XX:XX
        unsigned int b[6];
        char tail;
        if (sscanf(argv[2], "%2x:%2x:%2x:%2x:%2x:%2x%c",
                   &b[0], &b[1], &b[2], &b[3], &b[4], &b[5], &tail) != 6) {
            LOG_SIMPLE("Invalid MAC format (expect XX:XX:XX:XX:XX:XX): %s\r\n", argv[2]);
            return -1;
        }
        for (int i = 0; i < 6; i++) mac[i] = (uint8_t)b[i];
        if (factory_mac_is_valid(mac) != 0) {
            LOG_SIMPLE("Invalid MAC (must be unicast, not all-00 / all-FF): %s\r\n", argv[2]);
            return -1;
        }
        /* Burn first, reset identity only if the burn is durable: resetting
         * on a failed write would regenerate name/SSID/MQTT identity from a
         * MAC the next boot will not see, permanently mismatching them. */
        if (storage_nvs_write(NVS_FACTORY, NVS_KEY_MAC_ADDR, mac, sizeof(mac)) < 0) {
            LOG_SIMPLE("Failed to burn MAC (NVS write failed)\r\n");
            return -1;
        }
        factory_identity_reset(mac);
        LOG_SIMPLE("Factory MAC set: %02X:%02X:%02X:%02X:%02X:%02X\r\n",
                   mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
        LOG_SIMPLE("(takes effect on next boot; 'factory mac' to verify)\r\n");
        return 0;
    }
